...
- **BoD Wiki:** Collaborative knowledge base for admin and board members in Training page. Features include pinned entries, category filtering (general, policies, meetings, resources, training, contacts), full-text search, CRUD operations, and file attachments (PDF, Word, Excel, images, CSV/text up to 20MB). Attachments are stored on disk in `uploads/wiki/` with metadata in `wiki_attachments` table (cascade delete with disk cleanup). Stores meeting notes, procedures, institutional knowledge, and reference documents like tax exemption certificates.
...
- **Google Calendar API:** Custom OAuth2 integration with offline access and persistent refresh tokens stored in system_settings table. Two-way synchronization for event management with automatic token refresh. Admin connects via "Connect Indie Quill Collective Calendar" button in Calendar tab. OAuth state parameter prevents CSRF. Google Cloud project set to Production status for permanent tokens.
