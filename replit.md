...
- **Vibe Deck Curriculum System:** 4-tier hierarchy: Curriculum > Vibe Deck > Tome > Vibe Card. `curriculums` table (role-based paths: Published Writer, Adult Student, Family of Students), `vibe_decks` table (lesson groups within a curriculum), `tomes` table (instructional content within a deck - id, deck_id, title, content, order_index), `vibe_cards` table (individual writing tasks within a tome - references tome_id, not deck_id). Admin Vibe Deck Builder with full CRUD at all 4 levels, inline editing, and nested accordion UI. Student view shows published curricula with expandable decks > tomes > cards. Currently only Published Writer curriculum is being built with 3 TBD decks.
- **Tomes of Wisdom:** Tome-level educational content that gates card access. Each tome is a separate entity under a deck with its own title and content. Students must absorb each tome individually to unlock its cards. Scroll modal features parchment texture, center-out unrolling animation, CSS dowel bars, EB Garamond serif typography with ornate drop cap, ink-blue backdrop, and candlelight flicker animation. Tables: `tomes` (deck_id, title, content, order_index), `tome_absorptions` (user_id VARCHAR, tome_id, absorbed_at).
...
- **Card Submission Flow:** Self-report submission with brief reflection. Student writes manuscript, clicks "Submit for Review," provides reflection text, and earns XP. Card visually changes to completed state (green). Tables: `card_submissions` (user_id VARCHAR, card_id, manuscript_id, reflection, xp_earned, status, paste_count INTEGER, is_flagged_for_review BOOLEAN).
