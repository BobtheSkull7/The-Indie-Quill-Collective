...
import { Building2, DollarSign, Users, Target, Lock, TrendingUp, Plus, Globe, Mail, Phone, User, Calendar, X, Pencil, Trash2 } from "lucide-react";
  ...
  const handleCreateFoundation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Foundation create failed:", { status: res.status, data, sentBody: { name: newFoundation.name } });
        setMessage({ type: "error", text: data.message || "Failed to create foundation" });
      }
    } catch (err) {
      console.error("Foundation create error:", err);
  ...
  const handleEditFoundation = async (e: React.FormEvent, foundationId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        console.error("Foundation edit failed:", { status: res.status, data, foundationId });
        setMessage({ type: "error", text: data.message || "Failed to update foundation" });
      }
    } catch (err) {
      console.error("Foundation edit error:", err);
      setMessage({ type: "error", text: "Connection error" });
    } finally {
      setSaving(false);
    }
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-1">
                            ...
                            {authenticated && (
                              <>
                                <button
                                  onClick={() => startEditFoundation(foundation)}
                                ...
                                <button
                                  onClick={() => handleDeleteFoundation(foundation.id, foundation.name)}
                                ...
                                <button
                                  ...
                                  className="px-3 py-1 bg-teal-500 text-white rounded-lg hover:bg-teal-600 transition-colors text-sm font-medium flex items-center gap-1"
                                  title="Add a grant to this foundation"
                                >
